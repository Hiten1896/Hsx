...
function readChats() {
    try {
        const value = JSON.parse(localStorage.getItem(chatStorageKey) ?? '[]');
...
function saveChats() { localStorage.setItem(chatStorageKey, JSON.stringify(chats)); }
function activeChat() { return chats.find(chat => chat.id === activeChatId); }
function escapeHtml(value) { const node = document.createElement('div'); node.textContent = value; return node.innerHTML; }
function createChat() {
    const chat = { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, title: 'New Chat', messages: [] };
    chats.unshift(chat);
...
function renderChatList() {
    ...
    ui.chatList.querySelectorAll('.chat-row').forEach(row => {
...
function renderActiveChat() {
    ...
    chat.messages.forEach(message => renderMessage(message.role, message.text, false));
}
function showChatMenu(anchor, id) {
    document.querySelector('.menu')?.remove();
    const menu = document.createElement('div');
    menu.className = 'menu';
    menu.innerHTML = '<button data-action="rename">Rename</button><button data-action="delete">Delete</button>';
    anchor.append(menu);
...
async function sendMessage(event) {
    ...
    pending.innerHTML = '<div class="avatar">H</div><div class="bubble">Thinking...</div>';
    ui.conversation.append(pending);
    try {
        ...
        pending.remove();
        renderMessage('assistant', data.response ?? data.error ?? 'Something went wrong.');
        await loadFiles();
    }
    catch {
        pending.remove();
        renderMessage('assistant', 'Could not reach the local workspace server.');
    }
}
async function loadFiles() {
    const response = await fetch('/api/outputs');
    const data = await response.json();
    ui.artifactCount.textContent = data.files.length ? `${data.files.length} Active` : 'No artifacts';
    ui.emptyArtifact.hidden = data.files.length > 0;
    ui.files.innerHTML = data.files.length ? data.files.map(file => `<a href="/api/outputs/${encodeURIComponent(file.name)}" download>${escapeHtml(file.name)}</a>`).join('') : '<small>No generated files yet.</small>';
}
async function uploadFile(event) {
    ...
    const response = await fetch('/api/upload', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: file.name, content }) });
    const data = await response.json();
    renderMessage('assistant', data.success ? `Uploaded ${data.name}.` : data.error ?? 'Upload failed.');
    ui.file.value = '';
}
...
$('#title-menu').addEventListener('click', () => { const chat = activeChat(); if (chat)
    showChatMenu($('#title-menu').parentElement, chat.id); });
document.addEventListener('click', event => { if (!event.target.closest('.menu, .dots, #title-menu'))
    document.querySelector('.menu')?.remove(); });
let resizing = false;
const resize = $('#resize');
...
resize.addEventListener('pointermove', event => { if (resizing)
    $('#artifacts').style.flexBasis = `${Math.min(720, Math.max(300, innerWidth - event.clientX))}px`; });
resize.addEventListener('pointerup', () => { resizing = false; document.body.classList.remove('resizing'); });
function $(selector) { const node = document.querySelector(selector); if (!node)
    throw new Error(`Missing element: ${selector}`); return node; }
if (!chats.length)
    createChat();
else {
    renderChatList();
